...
image_names = os.listdir('/home/naveen/v3/Cs_Project/45/')
...
for images in tqdm(image_names):
    ...
    S = np.expand_dims(np.array(Image.open('/home/naveen/v3/Cs_Project/45/'+images).convert('L')),axis=-1)

    
    G = np.expand_dims(np.array(ImageOps.expand(Image.open('/home/naveen/v3/Cs_Project/Dataset/Test/Mask/mask'+images),border= 12,fill=0).convert('L')),axis=-1)
    print (S.shape)
    print (G.shape)
    print (images)
    image_name.append(str(images))
...
#a = 'Mean F1 : {}'.format(np.mean(np.array(mean_F1)))+ '\n'
f.write(str(a))
f.close()

import csv
resultFyle = open("Report_th50.csv",'wb')
for i in range(len(aggr_jacard)):
    resultFyle.write('%s,%f,%f,%f\n'%(str(image_name[i]),aggr_jacard[i],mean_F1[i],mean_dice[i]))
resultFyle.close()
